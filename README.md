## 이무진

### ~2026-04-19
과외 수강 기간 과제에 집중 한 후 Z-Blog 제대로 해보자

책도 매일 조금씩 읽어가자

### Blog
https://medium.com/@zkzkzhzj
 * Z-Blog 배포 이후 이전 예정

### Project & My Develop Log
https://github.com/zkzkzhzj/ChatAppProject

### Algorithm
https://github.com/zkzkzhzj/Algorithm

### 나의 목표
마플 - TypeScript/JavaScript, Node.js, PostgreSQL, 객체지향, 함수형, 탄탄한 기본기
 - 씨미 서비스, 크리에이터 산업
 - 예전에 좋아했고 지금 좋아하고 있는 산업, 제품에 대한 사랑/함께하고 싶은 분들

당근 - Local Business, Java/Kotlin, Spring, RESTFul API, DB 모델링, 캐시*트랜잭션, 자기주도적, 커뮤니케이션, 제품 사랑
 - 중고 거래, 커뮤니케이션, 내집 근처 사랑
 - 사람과 함께하는 서비스, 자사 제품을 사랑하는 분들

### 관심 블로그 찜
https://kciter.so/

https://medium.com/daangn
